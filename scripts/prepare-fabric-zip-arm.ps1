...
<#
.SYNOPSIS
Prepare Fabric-ready ZIP files with manifest.json + pipeline ARM Template JSON
.DESCRIPTION
Creates deployment-ready ZIPs for Microsoft Fabric import with correct ARM template format
#>
...
$LogFile = Join-Path $ProjectRoot "logs\zip-prepare-arm-$timestamp.log"
...
function Create-FabricZip {
    ...
    try {
        ...
        Log "Packaged: manifest.json, $PipelineName.json" "INFO"
...
Log "========================================" "INFO"
Log "FABRIC ZIP PREPARATION (ARM Template)" "INFO"
Log "========================================" "INFO"
...
if ($Pipeline -in @("emp", "all")) {
    Log "" "INFO"
    Log "--- Processing EMP Pipeline (ARM Template) ---" "INFO"
    $empZip = Join-Path $OutputDir "pl_m_poc_xml_emp_FABRIC.zip"
    $empManifest = Join-Path $SchemasDir "manifest_emp.json"
    $empJson = Join-Path $SchemasDir "pl_m_poc_xml_emp_FABRIC_DF_ARM.json"
...
if ($Pipeline -in @("hr", "all")) {
    Log "" "INFO"
    Log "--- Processing HR Pipeline (ARM Template) ---" "INFO"
    $hrZip = Join-Path $OutputDir "pl_m_poc_xml_hr_FABRIC.zip"
    $hrManifest = Join-Path $SchemasDir "manifest_hr.json"
    $hrJson = Join-Path $SchemasDir "pl_m_poc_xml_hr_FABRIC_DF_ARM.json"
...
Log "" "INFO"
Log "[OK] Ready for Fabric Import (ARM Templates):" "INFO"
